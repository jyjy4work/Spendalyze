...
export function AnomalyTable({ anomalies }: Props) {
  if (anomalies.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center py-16 text-slate-400">
        ...
        <p className="text-sm font-medium">이상 거래가 감지되지 않았습니다</p>
      </div>
    );
  }
  ...
  return (
    <div className="overflow-x-auto">
      <table className="w-full text-sm">
        <thead>
          <tr className="border-b border-slate-100 text-xs text-slate-400 uppercase tracking-wide">
            <th className="pb-2 pr-3 text-left font-medium">심각도</th>
            <th className="pb-2 pr-3 text-left font-medium">유형</th>
            <th className="pb-2 pr-3 text-left font-medium">날짜</th>
            <th className="pb-2 pr-3 text-left font-medium">카드소유자</th>
            <th className="pb-2 pr-3 text-left font-medium">가맹점</th>
            <th className="pb-2 pr-3 text-right font-medium">금액</th>
            <th className="pb-2 pr-3 text-left font-medium">계정과목</th>
            <th className="pb-2 pr-3 text-left font-medium">은행</th>
            <th className="pb-2 text-left font-medium">비고</th>
          </tr>
        </thead>
      ...
      <p className="text-xs text-slate-400 mt-3">총 {anomalies.length}건의 이상 거래 감지됨</p>
    </div>
  );
}
